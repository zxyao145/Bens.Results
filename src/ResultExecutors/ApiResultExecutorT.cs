...
internal sealed class ApiResultExecutor<T> : IActionResultExecutor<IApiResult<T>>
{
    ...
    private readonly IApiResultXmlSerializer<T> _apiResultXmlSerializer;

    public ApiResultExecutor(
        IOptions<JsonOptions> jsonOptions,
        IApiResultXmlSerializer<T> apiResultXmlSerializer)
    {
        _jsonOptions = jsonOptions.Value;
        _apiResultXmlSerializer = apiResultXmlSerializer;
    }

    public Task ExecuteAsync(ActionContext context, IApiResult<T> result)
    {
        ArgumentNullException.ThrowIfNull(context);
        return ExecuteAsync(context.HttpContext, result);
    }

    public async Task ExecuteAsync(HttpContext httpContext, IApiResult<T> result)
    {
        ...
        {
            var source = result.Data is not null
                ? Encoding.UTF8.GetBytes(result.Data.ToString() ?? string.Empty)
                : ReadOnlyMemory<byte>.Empty;

            await response.BodyWriter.WriteAsync(source, httpContext.RequestAborted);
